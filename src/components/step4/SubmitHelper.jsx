...
${invoiceData.businessName || 'Seller'}`;
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto">
            <div className="mb-10">
                <h2 className="text-[28px] font-black tracking-tight text-[#111827]">Prepare Submission</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10 mb-12">
                        ...
                        <div className="p-5 sm:p-10 space-y-8">
                            <div>
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button onClick={() => copyToClipboard(subjectText, 'subject')} className="text-[11px] font-bold text-[#2563EB] hover:underline underline-offset-4 decoration-2">
                                        {copiedStates.subject ? 'Copied' : 'Copy Subject'}
                                    </button>
                                </div>
                                <div className="p-5 bg-[#F9FAFB] rounded-xl border border-[#E5E7EB] text-[14px] font-bold text-[#111827]">{subjectText}</div>
                            </div>

                            <div>
                                <div className="flex justify-between items-center mb-3">
                                    <label className="text-[11px] font-bold text-[#6B7280] uppercase tracking-widest">Message Body</label>
                                    <button onClick={() => copyToClipboard(messageText, 'message')} className="text-[11px] font-bold text-[#2563EB] hover:underline underline-offset-4 decoration-2">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white rounded-[24px] shadow-sm border border-[#E5E7EB] p-8">
                        ...
                        <div className="space-y-4 text-[13px]">
                            <div className="flex justify-between">
                                ...
                                <span className="font-bold text-[#111827]">{invoiceData.invoiceNumber}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-[#6B7280] font-medium">Period</span>
                                <span className="font-bold text-[#111827]">{periodMatch}</span>
                            </div>
                            {invoiceData.sellerId && (
                                <div className="flex justify-between">
                                    <span className="text-[#6B7280] font-medium">Seller ID</span>
                                    <span className="font-bold text-[#111827]">{invoiceData.sellerId}</span>
                                </div>
                            )}
                            <div className="flex justify-between">
                            ...
                            <div className="pt-6 mt-2 border-t border-[#F3F4F6]">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="text-[13px] font-bold text-[#111827]">Claim Value</span>
                                    <span className="text-[18px] font-black text-[#16A34A]">{formatCurrency(stats.totalClaimValue)}</span>
                                </div>
                                <p className="text-[10px] text-[#9CA3AF] font-bold uppercase tracking-tight">Ready for submission</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[24px] shadow-sm border border-[#E5E7EB] p-8 flex items-center">
                        ...
                        <div>
                            <p className="text-[13px] font-bold text-[#111827]">Double Check</p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between pb-10 gap-4 border-t border-[#E5E7EB] pt-10 mt-10">
                <button 
                    onClick={onBack}
                    className="bg-white hover:bg-[#F3F4F6] text-[#111827] border border-[#D1D5DB] font-medium py-3.5 px-8 rounded-full shadow-sm flex items-center w-full sm:w-auto justify-center transition-all"
                >
